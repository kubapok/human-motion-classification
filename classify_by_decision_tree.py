...
while(1):
    ...
    if np.count_nonzero(fgmask) > VIDEO_WIDTH * VIDEO_HEIGHT / 3100: # 3100 is just parm tuned by experiments
        ...
        if lindex and rindex and dindex and uindex:
            ...
            predicted = tree_classifier.predict(
                [mean(v_motion_history), mean(h_motion_history), mean(ratio_history)])
            print(tree_classifier.classes_dict[predicted], '\t', float("{0:.2f}".format(mean(v_motion_history))), '\t', float(
                "{0:.2f}".format(mean(h_motion_history))), '\t', float("{0:.2f}".format(mean(ratio_history))), '\t')
